...
public class GlobalExceptionHandler {
    ...
    // MissingServletRequestParameterException is thrown when a required request
    // parameter is missing
    @ExceptionHandler(MissingServletRequestParameterException.class)
    public ResponseEntity<ErrorResponseDto> handleMissingServletRequestParameterException(
            MissingServletRequestParameterException exception, WebRequest webRequest) {
        ErrorResponseDto errorResponseDto = new ErrorResponseDto(
                webRequest.getDescription(false), HttpStatus.BAD_REQUEST,
                "Required parameter is missing: " + exception.getParameterName(), LocalDateTime.now());
